...
private:
   ...
   * Add any helper methods you may need
   */
  void Deallocate(int** table) const;
  std::string ToStringIntTable(int** table) const;
  int* GetOneVH(int** table) const;
  int* GetOneDH(int** table) const;
  int Delta(Pixel one, Pixel two) const;
};
